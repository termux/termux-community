...
- [Search first before posting](#search-first-before-posting) (others may have already posted about it)
...
When searching, like on Google, add the words `stackoverflow`, `stackexchange`, `github`, `reddit` or `xda` at the end of your query so that the search engine tries to prioritize the results from these sites, since these sites are often where you can get help for your technical questions. You can also search on these sites directly in the relevant forums, since search engine site results may not always show required results.
...
Additionally, search in the `Discussions` and `Pull requests` pages as well. In some cases, searching in the repo's git commit history or code can be useful too in case no issue or pull request was submitted for it. This can be done by using the search field of the repo at the top.
...
**If you did not find what you were looking for, only then**

- If you **have a question to ask**, then you can **create a new `Discussion`** on the relevant repo. If `Discussions` are not enabled for the repo, then you can create an `Issue` instead. Check GitHub's [Creating a discussion](https://docs.github.com/en/discussions/quickstart#creating-a-new-discussion) docs for more info.

- If you **have an issue/bug to report or have a new feature request** that you want implemented, then you can **create a new `Issue`** on the relevant repo. Check GitHub's [Creating an issue](https://docs.github.com/en/issues/tracking-your-work-with-issues/creating-an-issue) docs for more info.

- If you think you **have the capabilities to implement an issue/bug fix or feature request** that has not been implemented yet, then you can [fork](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/working-with-forks/about-forks) the repository into your own account and **create a new `Pull request`** to the upstream repo. However, before you send a `Pull request`, it is generally a good idea to create an `Issue` and ask the maintainers if you can implement it and how, as maintainers may want it implement it themselves or may already have implemented it locally, or may require implementation to be done in a specific way, or may not even approve of the changes that are to be implemented, or may think that you do not have sufficient skills to implement it yourself. Asking first would save your time and that of maintainers since you wouldn't waste time implementing something in a wrong way or something that would never get merged. Check GitHub's [Creating a pull request from a fork](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/proposing-changes-to-your-work-with-pull-requests/creating-a-pull-request-from-a-fork) docs for more info. **Only send a pull request if you have sufficient understanding of [`git`](https://git-scm.com/docs/git)** and how to rebase your pull branch against upstream `master` branch and how to edit or squash commits as that is neither the job of the maintainers to teach you nor do they often have time to fix such issues themselves.

Note that you will need a GitHub account logged in to create an `Issue`, `Discussion` or `Pull request`.

**All our community members must follow the rules that are defined [here](index.md) and any [Content Not Allowed](index.md#8-content-not-allowed) must not be posted.**
...
Include the thing you are actually trying to accomplish at the top level - in addition the problem you think you're stuck on. It is *extremely* common to spend a lot of time helping someone with a specific issue, only to find out later that they were going a completely wrong direction and it would have been obvious if they would had stated their main goal from the start.
